...
name = 'Isabella_lake_preproc_subim1'
...
hrmsi = sio.loadmat(raw_image_dir)['MSI'].transpose([2, 0, 1])
# Lets crop a smaller piece of this image
label = label[:, 8:88, 39:119] 
hrmsi = hrmsi[:, 8:88, 39:119]

kernel = gaussian_kernel_2d(scale_factor, kernel_sigma)
# kernel = np.ones((scale_factor,scale_factor))/(scale_factor**2)

srf = sio.loadmat(raw_image_dir)['SRF']
